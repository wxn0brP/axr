...
async function wake(
	mac: string,
	commandTemplate = DEFAULT_COMMAND,
): Promise<string> {
	...
	if (exitCode !== 0)
		throw new Error(
			stderr.trim() || `Wake command failed with exit code ${exitCode}`,
		);
...
export default (ctx: PluginCtx) => {
	...
	const deviceOptions = Object.entries(ctx.config?.devices || {}).map(
		([name]) => ({
			label: name,
			value: name,
		}),
	...
	ctx.adapter.add("send", async query => {
		...
		if (!mac)
			return {
				err: true,
				msg: "MAC address or device name is required",
			};
